...
class Solution:
    ...
    def simulate(self, num: int) -> str:
        ...
        for key in value_dicts.keys():
            if num // key != 0:
                count = num // key
                res += value_dicts[key] * count
                num %= key

        return res

    def violence(self, num: int) -> str:
    ...
    def intToRoman(self, num: int) -> str:
        # return self.simulate(num)
        return self.violence(num)


# @lc code=end

nums = [3749, 58, 1994]
ans = ["MMMDCCXLIX", "LVIII", "MCMXCIV"]
for num, a in zip(nums, ans):
    res = Solution().intToRoman(num)
    print(f"num = {num}, ans = {a}, res = {res}.")
